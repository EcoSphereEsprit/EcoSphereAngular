...
export class FlashSaleService {
    ...
    public baseUrl = "http://localhost:9090/flashSale"
    ...
    addFlashSale(flashSale: any): Observable<any> {
        ...
        formData.append('product', flashSale.product);
        formData.append('price', flashSale.price);
        formData.append('discountPrice', flashSale.discountPrice);
        formData.append('startDate', flashSale.startDate);
        formData.append('endDate', flashSale.endDate);
        formData.append('image', flashSale.image);

        return this.http.post<any>(`${this.baseUrl}/post`, formData, { headers });
    }
    ...
    // }
    getFlashSales(): Observable<any> {
        const token = localStorage.getItem("token");
        const headers = new HttpHeaders({
            'Authorization': `Bearer ${token}`
        });
        return this.http.get<any>(`${this.baseUrl}/findAll`, { headers });
    }
